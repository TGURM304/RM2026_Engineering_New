...
clear;
% 正解输 入关节角
%q_input = [127.57, 66.3, -123.82, -95.02, 80.84, 32.09];
q_input = [-114.15, 73.01, -130.99, 95.06, 74.65, -31.04];
%% 公式推导正解
% 连杆参数(没有的默认为零）
a2 = 0.430;
a3 = -0.0785; % 未知
...
T__ = transl(-0.129, 0.114, 0.362) * trotx(106.0) * troty(-51.9) * trotz(102.7);
%T__ = transl(-0.119, -0.117, 0.364) * trotx(-129.2) * troty(-64.7) * trotz(-124.2);
%                                       Y               P               R
T_ = transl(-0.111, -0.173, 0.398) * trotx(100.6) * troty(10.4) * trotz(87.2);
...
L2.qlim =[  -39.0 * pi / 180.0, 152.0 * pi / 180.0];
L3.qlim =[-137.0 * pi / 180.0,  18.0 * pi / 180.0];
L4.qlim =[-360.0 * pi / 180.0, 360.0 * pi / 180.0];
L5.qlim =[ -114.0 * pi / 180.0,  114.0 * pi / 180.0];
L6.qlim =[ -pi               ,  pi ];
...
target_point1 = [-0.130, -0.110, 0.350];
target_point2 = [-0.130,  0.110, 0.350];
